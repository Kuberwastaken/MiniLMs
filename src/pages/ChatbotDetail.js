...
const ChatbotFrame = styled(motion.div)`
  ...
  top: ${props => props.isFullscreen ? '70px' : 'auto'};
  left: ${props => props.isFullscreen ? '0' : 'auto'};
  right: ${props => props.isFullscreen ? '0' : 'auto'};
  bottom: ${props => props.isFullscreen ? '0' : 'auto'};
  z-index: ${props => props.isFullscreen ? '999' : '1'};
  height: ${props => props.isFullscreen ? 'calc(100vh - 70px)' : 'auto'};
  padding-top: ${props => props.isFullscreen ? '0' : '0'};
...
const ChatbotDetail = () => {
  ...
  const getIframeUrl = () => {
    ...
    if (process.env.NODE_ENV === 'development') {
      // For development, assumes a local HTTP server on port 8000 serves the project root.
      // chatbotInfo.path is like '/SYNEVA/SYNEVA.html'
      return `http://localhost:8000${chatbotInfo.path}`;
    } else {
      // For production (GitHub Pages)
      // process.env.PUBLIC_URL is '/MiniLMs' (from package.json homepage)
      // chatbotInfo.path is '/SYNEVA/SYNEVA.html'
      return process.env.PUBLIC_URL + chatbotInfo.path;
    }
  };
